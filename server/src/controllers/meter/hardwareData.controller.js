...
//     }
   ...
   if (aiPrediction !== undefined) {
      // Map IDs to specific actions based on AI Engine logic
      const statusMap = {
        1: "Water",
        2: "Fertilizer"
      };

      // If aiPrediction is 1 or 2, recommendation is "Water" or "Fertilizer".
      // Otherwise (if 0), it defaults to "Balanced (Normal)".
      const recommendation = statusMap[aiPrediction] || "Balanced (Normal)";

      const historyEntry = {
        timestamp: new Date(),
        title: "Soil Analysis Update",
        // Logic: if statusMap has the key, it's an intervention; otherwise, it's balanced.
        description: `New AI Prediction score: ${aiPrediction}. ${
          statusMap[aiPrediction] 
            ? `Plant needs: ${recommendation}` 
            : "Soil status: Balanced"
